...
TEST_F(CollectivePermuteDecomposerTest, SyncNotTransformed) {
  ...
  ENTRY test_computation {
    p = u32[] replica-id()
    permute = u32[] collective-permute(p), source_target_pairs={{0,0}, {1,1}, {2,2}, {3,3}}
    ROOT copy = u32[] copy(permute)
  }
  )";
...
TEST_F(CollectivePermuteDecomposerTest, SyncNotTransformed2) {
  const absl::string_view kModuleStr = R"(
      HloModule test
      ENTRY test_computation {
        p = u32[] replica-id()
        start = u32[] collective-permute-start(p), source_target_pairs={{0,1}, {1,2}}, backend_config="{\"is_sync\":true}"
...
TEST_F(CollectivePermuteDecomposerTest, WithCycleNotTransformed) {
  const absl::string_view kModuleStr = R"(
      ...
      ENTRY test_computation {
        p = u32[] replica-id()
        start = u32[] collective-permute-start(p), source_target_pairs={{0,1}, {1,0}}
        ROOT done = u32[] collective-permute-done(start)
      }
    )";
...
TEST_F(CollectivePermuteDecomposerTest, ComplilcateResultTypeNotTransformed) {
  const char* const kModuleStr = R"(
  HloModule test
  ENTRY test_computation {
    p = u32[] replica-id()
    start = (u32[], u32[]) collective-permute-start(p), source_target_pairs={{0,1}, {1,2}, {2,3}, {3,4}}
    ROOT done = u32[] collective-permute-done(start)
  }
  )";
...
TEST_F(CollectivePermuteDecomposerTest, TransformedDefaultChannelId) {
  ...
  ENTRY test_computation {
    p = u32[] replica-id()
    start = u32[] collective-permute-start(p), source_target_pairs={{0,1}, {1,2}, {2,3}, {3,4}}
    ROOT done = u32[] collective-permute-done(start)
  }
  )";
...
TEST_F(CollectivePermuteDecomposerTest, TransformedExplicitChannelId) {
  ...
  ENTRY test_computation {
    p = u32[] replica-id()
    start = u32[] collective-permute-start(p), channel_id=2, source_target_pairs={{0,1}, {1,2}, {2,3}, {3,4}}
    ROOT done = u32[] collective-permute-done(start)
  }
  )";
