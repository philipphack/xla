  ...
  if (chunk_candidate) {
    ...
    if (prev_allocation != nullptr &&
        (prev_allocation->is_copy_allocation() ||
         prev_allocation->is_sliced_copy_allocation() ||
...
MemorySpaceAssignment::SliceDecision::ToTuple() const {
  return std::make_tuple(std::ref(chunk), start_time, std::ref(sizing),
                         copy_resource_consumed);
}
...
MemorySpaceAssignment::SlicedCopyAllocation::SliceDetail::ToTuple() const {
  return std::make_tuple(std::ref(slice_decision), copy_start_after_time,
                         copy_done_before_time, copy_start, copy_done);
}
