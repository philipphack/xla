...
std::optional<WindowedEinsumConfig> GetWindowedEinsumConfiguration(
  ...
  // Partially windowed case with multiple sharded operand dimensions as seen in
  ...
  if (lhs_non_contracting_partitions == num_partitions &&
      ...
      partitioned_lhs && options.partial_windowed_einsum) {
    TileAssignment lhs_tile_assignment = lhs_sharding.tile_assignment();

    // The LHS has exactly one sharded non-contracting dimension that
    // maps to an identically sharded dimension in the output. Additionally,
    // the LHS has exactly one sharded non-contracting dimension that maps to a
    // replicated dimension in the output which becomes the dimension of the
    ...
    if (lhs_windowing_dim_count != 1 ||
      ...
      VLOG(3)
          << "Partially windowed case requires exactly one LHS windowing "
    ...
    for (int64_t k = 0;
      ...
      for (int64_t l = 1; l < lhs_tile_assignment.dim(lhs_windowing_dim); ++l) {
        ...
        if (lhs_tile_assignment(indices) < prev_tile_assignment) {
          VLOG(3)
              << "Partially windowed case requires monotonic tile assignment.";
          return std::nullopt;
        }
    ...
    // non-contracting dimensions of the LHS and RHS by merging their respective
    // shardings. Set the sharding of the windowing dimension of the LHS to
    // replicated for the purpose of merging with the RHS and move the
    // corresponding devices to partial replication.
    ...
    // Pad the tile assignment of the RHS for the purpose of merging the
    // sharding with the LHS.
