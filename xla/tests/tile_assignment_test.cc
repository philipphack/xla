...
class FormatedTileAssignmentTest : public ::testing::TestWithParam<bool> {
 protected:
  bool ShouldConvertToV1() { return GetParam(); }
};

TEST_P(FormatedTileAssignmentTest, TrivialIotaTile) {
...
TEST_P(FormatedTileAssignmentTest, TransposedIotaTile) {
...
TEST_P(FormatedTileAssignmentTest, NonCanonicalTransposedIotaTile) {
...
TEST_P(FormatedTileAssignmentTest, ReshapeTrivalIotaTile) {
  TileAssignment tile({4, 4, 2});
  if (ShouldConvertToV1()) {
    tile = TileAssignment(tile.shared_array());
  }
...
TEST_P(FormatedTileAssignmentTest, ReshapeTransposedIotaTile) {
  TileAssignment tile({4, 4, 2}, {2, 4, 4}, {2, 1, 0});
  if (ShouldConvertToV1()) {
    tile = TileAssignment(tile.shared_array());
  }
...
TEST_P(FormatedTileAssignmentTest, TransposeTrivalIotaTile) {
  TileAssignment tile({4, 4, 2});
  if (ShouldConvertToV1()) {
    tile = TileAssignment(tile.shared_array());
  }
...
TEST_P(FormatedTileAssignmentTest, TransposeTransposedIotaTile) {
  TileAssignment tile({4, 4, 2}, {2, 4, 4}, {2, 1, 0});
  if (ShouldConvertToV1()) {
    tile = TileAssignment(tile.shared_array());
  }
...
TEST_P(FormatedTileAssignmentTest, TransposeIotaTileWithDegernateDims) {
...
INSTANTIATE_TEST_SUITE_P(All, FormatedTileAssignmentTest, ::testing::Bool());
